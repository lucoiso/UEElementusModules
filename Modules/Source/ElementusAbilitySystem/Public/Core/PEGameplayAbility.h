...
class ELEMENTUSABILITYSYSTEM_API UPEGameplayAbility : public UGameplayAbility
{
	GENERATED_BODY()
...
protected:
	...
	*/
	...
	void BP_SpawnProjectileWithTargetEffects(const TSubclassOf<APEAbilityProjectile> ProjectileClass, const FTransform ProjectileTransform, const FVector ProjectileFireDirection);

	void SpawnProjectileWithTargetEffects(TSubclassOf<APEAbilityProjectile> ProjectileClass, const FTransform ProjectileTransform, const FVector ProjectileFireDirection, const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo);

	/* Default callback for SpawnProjectileWithTargetEffects function */
	UFUNCTION(BlueprintNativeEvent, Category = "Project Elementus | Functions | Callbacks")
	void SpawnProjectile_Callback(APEAbilityProjectile* SpawnedProjectile);

	virtual void SpawnProjectile_Callback_Implementation(APEAbilityProjectile* SpawnedProjectile)
	{
	} // Override this function on children classes.
